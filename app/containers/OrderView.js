...
class OrderView extends Component{
	...
	componentWillMount(){
		...
		let maxId = parseInt(cartIdList[0]);
		...
		const passwordToken = 'Bearer ' + passwordTokenResult.access_token;
	...
	componentWillReceiveProps(nextProps){
		...
		if (orderFailed &&  orderLoading == true) {
			this.showAlert('Error', orderFailed.error);
		}
	...
	formatPhoneNumber (phone_number) {
	  let x = phone_number.replace(/\D/g, '').match(/(\d{0,3})(\d{0,3})(\d{0,4})/);
	...
	setDriverTip(cardID, tipList, driver_tip) {
		let {key, totalDriverTipList} = this.state;
		let value = driver_tip;
		if (key != 'Backspace')
		{
		...
		if (value > 25)
			value = 25;
		let num_str = numeral(value).format('0.00').toString();
	...
	getAddressInfo(details) {
        ...
        let route, country, administrative_area_level_1, locality, postal_code, street_number;
	...
	_onCheckOut(totalDriverTipList){
		this.getDeviceData();
		let {deliveryDataArr, userGeoAddressList, userBillingGeoAddressList, cartInfo, createcartInfo} = this.props;
		let { promo_code, delivery_instruction, passwordToken, unit_number, same_with_ship} = this.state;
    
		const credit_card = this.state.orderCardSelectID;
		let deliveryAddress = this.getAddressInfo(userGeoAddressList.userGeoAddress);
		let cart_products = {};
		let cart_supplier = [];
		let tipTotal = {};
		let deliveryDate = {};
		let deliveryTime = {};
		let cart_id = createcartInfo.id;
		if (cartInfo) {
			for (i = 0; i < cartInfo.length; i ++) {
		...
		// 	}
		// }
		let add_new_delivery_address = 1;
		let new_delivery_address = userGeoAddressList.userGeoAddress.formatted_address;
		let new_delivery_address_apt_suite = unit_number;
		let add_new_billing_address = 1;
		let new_billing_address = '';
		if (same_with_ship == true)
		{
			new_billing_address = userGeoAddressList.userGeoAddress.formatted_address;
		}
		if (userBillingGeoAddressList && same_with_ship != true) {
			new_billing_address = this.state.billingAddress;
		}
		
		let new_billing_apt_suite = 3;
		let device_data = null;
		let str_alert_message = '';
		let alert_coming = false;
		...
		if (new_billing_address == '' || new_billing_address == null) {
		...
		const {loadingOfPlaceOrder} = this.props;
		
		if (loadingOfPlaceOrder == false)
	...
	render(){
		...
		let totalDriverTip = 0, imgUrl = null, orderSubTotal = 0, supplierTotal = 0, delivery_fee = 0, tax_rate = 0;
		...
		let x = Math.floor(tax_rate * 100) / 100;
		tax_rate = x.toFixed(2);
		let orderSubTotal1 = 0;
		if (discount == null) {
			 discount = 0;
		...
		let orderTotal = parseFloat(orderSubTotal1) + parseFloat(delivery_fee) + parseFloat(tax_rate) + parseFloat(totalDriverTip);
			...
			<SideMenu
				...
				{cartInfo != null ? 
					...
					: null
					  	...
					  	<TouchableOpacity onPress={()=>this._onCheckOut(totalDriverTipList)} style={styles.checkoutBtn}>
