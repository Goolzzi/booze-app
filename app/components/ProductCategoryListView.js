...
import {savePrevPage, saveCurrentPage, saveTabType, saveIsDetail} from '../redux/actions/tabInfoAction';
...
let ListViewHeight = screen_height*0.22*7;
		...
		return(
			<SideMenu
				...
				<View style={styles.bg}>
				  	<SearchBarView addFlag='true' onPress={()=>this.toggle()} orderPage='true'/>
	...
	_renderRow (rowData: object, sectionID: number, rowID: number, highlightRow: (sectionID: number, rowID: number) => void) {
	    return (
	      <TouchableHighlight onPress={()=>{highlightRow(sectionID, rowID); this._onClickProductCategory(rowData)}}>
	        <View style={{height: (screen_height-75)*0.25 - 2}}>
				{(rowData.banner != "") && (
				...
				{!rowData.banner &&  rowData.title.toLowerCase() != "party orders" && rowData.title.toLowerCase() != "featured" &&(
	        	...
	        	{!rowData.banner && rowData.title.toLowerCase() == "party orders" &&(
	        		<Image source={require('../assets/images/boozeapp_party.jpg')} resizeMode={Image.resizeMode.contain} style={styles.categoryList_image}>
						<View style={{flexDirection:'column',flex:1}}>
							<View style={{flex:1,justifyContent:'center',marginLeft:27}}>
							   	<Text style={styles.categoryListTitle}>{rowData.title.toUpperCase()}</Text>
							</View>
	        	...
	        	{!rowData.banner && rowData.title.toLowerCase() == "featured" &&(
	        		<Image source={require('../assets/images/boozeapp_featured.jpg')} resizeMode={Image.resizeMode.contain} style={styles.categoryList_image}>
						<View style={{flexDirection:'column',flex:1}}>
							<View style={{flex:1,justifyContent:'center',marginLeft:27}}>
							   	<Text style={styles.categoryListTitle}>{rowData.title.toUpperCase()}</Text>
							</View>
...
export default connect(
  ...
  }),{getCartInfo, loadProductCategoriesList, setProductCategory, savePrevPage, saveCurrentPage, saveTabType, saveIsDetail, createCartInfo})(ProductCategoryListView);
